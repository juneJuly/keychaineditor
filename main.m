...
OSStatus osstatusToHumanReadableString(OSStatus status) {
    ...
    switch (status) {
        ...
        case errSecAuthFailed:
            fprintf(stderr, "Did you turn off the passcode on device? The item is no longer available.\n");
            fprintf(stderr, "If that is not the case, UserPresence is required. Check your device for the prompt.\n");
            break;
        case -34018:
            fprintf(stderr, "Error: Please codesign this keychaineditor first.\n");
            break;
        case -25308:
            fprintf(stderr, "Error: Please unlock your iphone first.\n");
            break;
            
        default:
            fprintf(stderr, "Unhandled Error: Please contact developer to report this error. Error code: %d\n", (int)status);
            break;
    }
...
NSString* determineTypeAndReturnNSString(id accountValue) {
    ...
    if ([accountValue isKindOfClass:[NSData class]]) {
        NSString *result = [[NSString alloc] initWithData:accountValue encoding:NSUTF8StringEncoding];
        if (!result) {
...
void prepareJsonOutput(NSDictionary *results, NSString *find) {
    ...
    if (json != nil && error == nil) {
        ...
        fprintf(stderr, "\nInternal error when preparing JSON output. Error: %s\n", [[error localizedDescription] UTF8String]);
    }
}
...
void prepareMinimumOutput(NSDictionary *results) {
    ...
    fprintf(stdout, "Warning: The names are truncated to max width of 35 charaters. Please use this dump as a reference, and use --find to get full details.\n");
...
int main(int argc, char *argv[]) {
    @autoreleasepool {
        ...
        if (![[NSString stringWithFormat:@"%s", argv[1]] isEqualToString:@"--action"]) {
            fprintf(stderr, "Invalid command passed as first argument. Please use --help to get usage information.");
            return EXIT_FAILURE;
        }
        ...
        if (![@[@"min-dump", @"dump", @"edit", @"delete"] containsObject:action]) {
            fprintf(stderr, "Invalid action passed. Please use --help to get usage information.\n");
            return EXIT_FAILURE;
        }
        ...
        if ([action isEqualToString:@"dump"] || [action isEqualToString:@"min-dump"]) {
            ...
            if ( (acct == nil) || (srvc == nil) || (data == nil) ) {
                fprintf(stderr, "Edit requires account, service and data. Please use --help to get usage information.\n");
                return EXIT_FAILURE;
            }
            ...
            if (decodedData == nil) {
                fprintf(stderr, "Data only accepts base64 strings. Please use --help to get usage information.\n");
                return EXIT_FAILURE;
            }
            ...
            if ( (acct == nil) || (srvc == nil) ) {
                fprintf(stderr, "Delete requires account, service. Please use --help to get usage information.\n");
                return EXIT_FAILURE;
            }
